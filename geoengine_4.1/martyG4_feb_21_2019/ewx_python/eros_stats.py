# -*- coding: utf-8 -*-'
# Modified: Aug 3, 2018  cholen - Add in geom_leeway.
...
VALID_STATS = DEFAULT_STATS + \
    ['sum', 'std', 'median', 'majority', 'minority', 'unique', 'range','rawpixels']

def raster_stats(vectors, raster, layer_num=0, band_num=1, nodata_value=None, exclude_ranges=None, 
    ...
    for i, feat in enumerate(features_iter):
        ...
        # There is a problem when the polygon extents extends outside
        # the raster extents, it can cause the ReadAsArray calls used later
        # to return None.  To fix this, we need to adjust how we set the
        # polygon extents when the actual extents go outside the raster.
        # Instead of setting the edge of the polygon to the edge of the
        # raster, we set it to the edge of the raster + some small leeway
        # amount(0.0001).
        geom_leeway = 0.0001
        geom_bounds = list(geom.bounds)
        if geom_bounds[0] < rbounds[0]:
            geom_bounds[0] = rbounds[0] + geom_leeway
        if geom_bounds[1] < rbounds[1]:
            geom_bounds[1] = rbounds[1] + geom_leeway
        if geom_bounds[2] > rbounds[2]:
            geom_bounds[2] = rbounds[2] - geom_leeway
        if geom_bounds[3] > rbounds[3]:
            geom_bounds[3] = rbounds[3] - geom_leeway
        ...
        if src_offset[2] <= 0 or src_offset[3] <= 0:
            ...
            if exclude_ranges is not None:
                 ers = exclude_ranges.split()             
                 for range in ers:
                      nodata_values = range.split(',')
                      #nodata_value_min = int(nodata_values[0])
                      #nodata_value_max = int(nodata_values[1])
                      nodata_value_min = float(nodata_values[0])
                      nodata_value_max = float(nodata_values[1])
                      # replace any nans with the no data value
                      src_array[np.isnan(src_array)] = nodata_value_min
                      places_to_mask = np.logical_or(np.logical_and(src_array >= nodata_value_min, src_array <= nodata_value_max),places_to_mask) 

            masked = np.ma.masked_where(places_to_mask,src_array)
			
            #print str(masked)[1:-1]
            #print '\n'
            ...
            if 'rawpixels' in stats:
                feature_stats['rawpixels'] = masked.compressed()
